...
describe("saml", () => {
  it("should sign and verify a xml signature", async () => {
    const xmlContent =
      "<samlp:Response><saml:Issuer>https://keycloak.rejlers-srv01.se/auth/realms/master</saml:Issuer></samlp:Response>";
    ...
    const signedXmlString = xmlContent.replace(
      "</samlp:Response>",
      signatureXml + "</samlp:Response>",
    );
    const signedDocument = XmlDSigJs.Parse(signedXmlString);
    const signedXml = new XmlDSigJs.SignedXml(signedDocument);
  ...
  it("should sign and xml using xml-crypto with sha-1", async () => {
    const xmlContent =
    ...
    const sig = new SignedXml({ privateKey: privateKeyPem, publicCert: cert });
    sig.addReference({
      xpath,
