...
const usersToUpdate: { id: string; email: string }[] = [];
...
async function addUser(user: User) {
  for await (const identity of removeNonUniqueByKey(user.identities)) {
    if (
      ...
      identity.profileData.email !== user.email
    ) {
      console.log(
        `Email mismatch: ${identity.profileData.email} !== ${user.email}. User: ${identity.provider}|${identity.user_id}`,
      );
      usersToUpdate.push({
        id: `${identity.provider}|${identity.user_id}`,
...
async function fixEmailUsers(filePath: string) {
  try {
...
fixEmailUsers("./data/sesamy-prod.json");
